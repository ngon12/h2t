...
class PortfolioList implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
        if(function_exists('vc_map')) {
            ...
            if(qode_core_theme_installed()) {
                $icons_array = \KloeQodefIconCollections::get_instance()->getVCParamsArray();
            }
    ...
    public function render($atts, $content = null) {
		...
		$slider_class="";
		if(!empty($params['portfolio_slider']) && $params['portfolio_slider'] == 'yes' && $type == "fullscreen-slider"){
			$slider_class .= 'qodef-portfolio-full-screen-slider';
		}
		
		$html .= '<div class = "qodef-portfolio-list-holder clearfix '.$slider_class.'" >';
		...
		if($query_results->have_posts()):			
			...
			while ($i <= $columns_num) {
				$i++;
				if ($columns != 1) {
					if(empty($params['portfolio_slider']) && $type != 'masonry') {
    ...
    */
	public function getPortfolioIconsHtml($params){
		...
		if (function_exists('kloe_qodef_like_portfolio_list')) {
			$html .= kloe_qodef_like_portfolio_list($id);
		}
		
		$html .= '<a class="qodef-preview" title="Go to Project" href="' . $this->getItemLink($params) . '" data-type="portfolio_list"></a>';
	...
	public function getPortfolioClasses($params){
		...
		switch($type):
			...
			case 'fullscreen-slider':
				$classes[] = 'qodef-full-screen-slider-holder';
			break;
		endswitch;
	...
	public function getImageSize($params){
		...
		if($type == 'standard' || $type == 'gallery'){
			if(!empty($params['image_size'])){
				...
				switch ($image_size) {
					case 'landscape':
						$thumb_size = 'kloe_qodef_landscape';
						break;
					case 'portrait':
						$thumb_size = 'kloe_qodef_portrait';
						break;
					case 'square':
						$thumb_size = 'kloe_qodef_square';
						break;
					case 'full':
						$thumb_size = 'full';
						break;
		...
		elseif($type == 'masonry'){
			...
			switch($masonry_size):
				case 'default' : 
					$thumb_size = 'kloe_qodef_portfolio_square';
				break;
				case 'large_width' : 
					$thumb_size = 'kloe_qodef_large_width';
				break;
				case 'large_height' : 
					$thumb_size = 'kloe_qodef_large_height';
				break;
				case 'large_width_height' : 
					$thumb_size = 'kloe_qodef_large_width_height';
				break;
			endswitch;
		}
    ...
    */
	public function getDataAtts($params){
		...
		if(!empty($params['type']) && $params['type'] == 'gallery' && !empty($params['portfolio_slider']) && $params['portfolio_slider']=='yes'){
