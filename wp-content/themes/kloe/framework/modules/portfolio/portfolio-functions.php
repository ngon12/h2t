...
if(!function_exists('kloe_qodef_single_portfolio')) {
    function kloe_qodef_single_portfolio() {
        $portfolio_template = kloe_qodef_get_portfolio_single_type();
        ...
        if ($portfolio_template == 'gallery') {
            $params['holder_class'][] = 'qodef-portfolio-gallery-' . kloe_qodef_options()->getOptionValue('portfolio_single_numb_columns');
        }

        kloe_qodef_get_module_template_part('templates/single/holder', 'portfolio', '', $params);
    }
}

if(!function_exists('kloe_qodef_get_portfolio_single_type')) {
    function kloe_qodef_get_portfolio_single_type() {
        return kloe_qodef_get_meta_field_intersect('portfolio_single_template');
    }
}

if(!function_exists('kloe_qodef_get_portfolio_single_media')) {
    function kloe_qodef_get_portfolio_single_media() {
        ...
        if(is_array($videos) && count($videos)) {
            usort($videos, 'kloe_qodef_compare_portfolio_videos');
            foreach($videos as $video) {
                ...
                if(!empty($video['portfoliovideotype'])) {
                    ...
                    $media['video_url']   = kloe_qodef_portfolio_get_video_url($video);
...
if(!function_exists('kloe_qodef_portfolio_get_video_url')) {
    function kloe_qodef_portfolio_get_video_url($video) {
...
if(!function_exists('kloe_qodef_portfolio_get_media_html')) {
    function kloe_qodef_portfolio_get_media_html($media) {
        global $wp_filesystem;
        $params = array();

        //For adding image overlay in gallery template type
        $params['gallery'] = (kloe_qodef_get_portfolio_single_type() == 'gallery') ? true : false;

        if($media['type'] == 'image') {
            $params['lightbox'] = kloe_qodef_options()->getOptionValue('portfolio_single_lightbox_images') == 'yes';
        ...
        if(in_array($media['type'], array('youtube', 'vimeo'))) {
            $params['lightbox'] = kloe_qodef_options()->getOptionValue('portfolio_single_lightbox_videos') == 'yes';
        ...
        kloe_qodef_get_module_template_part('templates/single/media/'.$media['type'], 'portfolio', '', $params);
    }
}

if(!function_exists('kloe_qodef_compare_portfolio_videos')) {
    /**
     ...
     */
    function kloe_qodef_compare_portfolio_videos($a, $b) {
...
if(!function_exists('kloe_qodef_compare_portfolio_options')) {
    /**
     ...
     * @return int result of comparison
     */
    function kloe_qodef_compare_portfolio_options($a, $b) {
...
if(!function_exists('kloe_qodef_portfolio_get_info_part')) {
    function kloe_qodef_portfolio_get_info_part($part) {
        $portfolio_template = kloe_qodef_get_portfolio_single_type();

        kloe_qodef_get_module_template_part('templates/single/parts/'.$part, 'portfolio', $portfolio_template);
    }
}