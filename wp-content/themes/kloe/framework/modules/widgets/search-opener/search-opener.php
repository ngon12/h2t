 ...
 */
class KloeQodefSearchOpener extends KloeQodefWidget {
    /**
    ...
    public function widget($args, $instance) {
        global $kloe_qodef_options, $kloe_qodef_IconCollections;

        $search_type_class    = 'qodef-search-opener';
		$fullscreen_search_overlay = false;
        $search_opener_styles = array();
        $show_search_text     = $instance['show_label'] == 'yes' || $kloe_qodef_options['enable_search_icon_text'] == 'yes' ? true : false;
		$close_icon_on_same_position = $instance['close_icon_position'] == 'yes' ? true : false;


        if(isset($kloe_qodef_options['search_type']) && $kloe_qodef_options['search_type'] == 'search_covers_header') {
            if(isset($kloe_qodef_options['search_cover_only_bottom_yesno']) && $kloe_qodef_options['search_cover_only_bottom_yesno'] == 'yes') {
        ...
        <a <?php echo kloe_qodef_get_inline_attr($instance['search_icon_hover_color'], 'data-hover-color'); ?>
			<?php if ( $close_icon_on_same_position ) {
				echo kloe_qodef_get_inline_attr('yes', 'data-icon-close-same-position');
			} ?>
            <?php kloe_qodef_inline_style($search_opener_styles); ?>
            <?php kloe_qodef_class_attribute($search_type_class); ?> href="javascript:void(0)">
            <?php if(isset($kloe_qodef_options['search_icon_pack'])) {
                $kloe_qodef_IconCollections->getSearchIcon($kloe_qodef_options['search_icon_pack'], false);
            } ?>
            <?php if($show_search_text) { ?>
                <span class="qodef-search-icon-text"><?php esc_html_e('Search', 'kloe'); ?></span>
            <?php } ?>
        </a>
