<?php
namespace KloeQodef\Modules\Shortcodes\Icon;

use KloeQodef\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package KloeQodef\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
        ...
        vc_map(array(
            ...
            'params'                    => array_merge(
                ...
                array(
                    ...
                    array(
                        'type'        => 'colorpicker',
                        'heading'     => 'Hover shader background color',
                        'param_name'  => 'hover_effect_background_color',
                        'admin_label' => true,
                        'value'       => '#e4e4e4',
                        'save_always' => true,
                        'dependency'  => array('element' => 'hover_effect', 'value' => array('yes'))
                    ),
                    array(
                        'type'        => 'textfield',
                        'heading'     => 'Border radius',
                        'param_name'  => 'border_radius',
                        'description' => esc_html__('Please insert border radius(Rounded corners) in px. For example: 4 ', 'kloe'),
                    ...
                    array(
                        ...
                        'param_name'  => 'margin',
                        'admin_label' => true,
                        'description' => 'Margin (top right bottom left) - for example: 0px 10px 0px 0px'
                    ),
                    array(
                        'type'        => 'dropdown',
    ...
    public function render($atts, $content = null) {
        ...
        if($params['hover_effect'] == 'yes') {
            $iconHolderClasses[] = 'qodef-icon-hover-effect';
        }
        ...
        $params['icon_shader_styles']    = $this ->generateIconShaderStyles($params);
        ...
        $html = kloe_qodef_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
        ...
        if(($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
           ($params['type'] == 'normal')
            ...
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.kloe_qodef_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.kloe_qodef_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.kloe_qodef_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.kloe_qodef_filter_px($shapeSize).'px';
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
				...
				$iconHolderStyles[] = 'border-width: '.kloe_qodef_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.kloe_qodef_filter_px($params['border_radius']).'px';
                }
            }
        }
        else {
    ...
    /**
     * Generates icon shader styles
     *
     * @param $params
     *
     * @return string
     */
    private function generateIconShaderStyles($params) {
        $iconShaderStyles = array();

        if(!empty($params['hover_effect_background_color'])) {
            $iconShaderStyles[] = 'background-color: '.$params['hover_effect_background_color'];
        }

        return implode(';', $iconShaderStyles);
    }
}