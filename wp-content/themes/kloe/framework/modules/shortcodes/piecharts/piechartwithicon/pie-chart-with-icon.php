<?php
namespace KloeQodef\Modules\PieCharts\PieChartWithIcon;
...
class PieChartWithIcon implements ShortcodeInterface {
			...
			'params' => array_merge(
				...
				array(
					array(
						...
						'dependency' => Array('element' => 'icon_pack', 'value' => kloe_qodef_icon_collections()->getIconCollectionsKeys()),
						'group' => 'Design Options',
					),
					array(
						'type' => 'textfield',
	...
	public function render($atts, $content = null)
	{
		...
		$html = kloe_qodef_get_shortcode_module_template_part('templates/pie-chart-with-icon', 'piecharts/piechartwithicon', '', $params);
	...
	private function getPieChartData($params) {
		...
		else if(kloe_qodef_options()->getOptionValue('first_color') !== "") {
			$pieChartData['data-active-color'] = kloe_qodef_options()->getOptionValue('first_color');
		}
		else {
			$pieChartData['data-active-color'] = '#ea9195';
		}
	...
	private function getPieChartIcon($params) {
		...
		$pie_chart_icon = kloe_qodef_icon_collections()->renderIcon( $params[$icon], $params['icon_pack'], $iconStyles );
