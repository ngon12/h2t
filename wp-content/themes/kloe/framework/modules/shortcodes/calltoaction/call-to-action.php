<?php
namespace KloeQodef\Modules\CallToAction;
...
class CallToAction implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		$call_to_action_button_IconCollections = kloe_qodef_icon_collections()->iconCollections;
		...
		vc_map( array(
				...
				'params' => array_merge(
					...
					kloe_qodef_icon_collections()->getVCParamsArray(array('element' => 'type', 'value' => array('with-icon'))),
					array(
						array(
							'type' 			=> 'textfield',
						...
						array(
							...
							'value' => array_merge(array('No Icon' => ''),kloe_qodef_icon_collections()->getIconCollectionsVC()),
							'save_always' => true,
							'dependency' => array('element' => 'show_button', 'value' => array('yes'))
						)
					),
	...
	public function render($atts, $content = null) {
		...
		foreach (kloe_qodef_icon_collections()->iconCollections as $collection_key => $collection) {

			$call_to_action_icons_form_fields['button_' . $collection->param ] = '';

		}

		$args = array_merge($args, kloe_qodef_icon_collections()->getShortcodeParams(),$call_to_action_icons_form_fields);
		...
		$params['holder_background'] = $this->getCallToActionBackground($params);

		//Get HTML from template
		$html = kloe_qodef_get_shortcode_module_template_part('templates/call-to-action-template', 'calltoaction', '', $params);
	...
	private function getCallToActionIcon($params) {
		...
		$icon = kloe_qodef_icon_collections()->getIconCollectionParamNameByKey($params['icon_pack']);
		$iconStyles = array();
		$iconStyles['icon_attributes']['style'] = $this->getIconStyles($params);
		$call_to_action_icon = '';
		if(!empty($params[$icon])){			
			$call_to_action_icon = kloe_qodef_icon_collections()->renderIcon( $params[$icon], $params['icon_pack'], $iconStyles );
		}
	...
	private function getButtonParameters($params) {
		...
		if(!empty($params['button_icon_pack'])) {
			...
			$iconPackName = kloe_qodef_icon_collections()->getIconCollectionParamNameByKey($params['button_icon_pack']);
	...
	private function getCallToActionBackground($params) {
		$styles = array();
